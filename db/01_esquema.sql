...
-- ### 012_iniciativas
-- ###########################################################################
-- El perfil de una iniciativa deja de ser una ficha. Copia literal de
-- db/05_iniciativas.sql, que es el mismo bloque para aplicar encima de una base que ya
-- corrió este archivo antes de que existiera la sección. Si cambias uno, cambia el otro.
--
-- Campañas con meta concreta, agenda de actividades y entradas que cuentan lo ya hecho,
-- atadas a la campaña que lo pagó. El rol «centro» va en `center_managers` y NO en
-- `staff_users`: alcanza un punto, el suyo, y `is_staff()` sigue significando lo que
-- significaba.
--
-- La regla del dinero: la plataforma no cobra ni custodia. `raised_amount` es lo que la
-- iniciativa DICE que lleva; se guarda con quién y cuándo lo declaró, y la app está
-- obligada a mostrarlo como cifra declarada.
-- ===========================================================================
...
-- ### 013_reconocimiento
-- ###########################################################################
-- Medallas y tabla de posiciones. Copia literal de db/06_reconocimiento.sql, que es el
-- mismo bloque para aplicar encima de una base que ya corrió este archivo antes de que
-- existiera la sección. Si cambias uno, cambia el otro.
--
-- LA REGLA QUE GOBIERNA TODO ESTE BLOQUE: lo que se puede ver de otra persona es un
-- NÚMERO y un NOMBRE, nunca un lugar. Una tabla de posiciones hecha sobre visitas a
-- refugios diría dónde ha estado la gente, que es justo lo que `010_accounts` prohíbe
-- leer de `favourites` incluso a un superadmin.
-- ===========================================================================
...
-- ### 018_horario
-- ###########################################################################
-- El horario de atención marcado día por día. Copia literal de db/12_horario.sql, que
-- explica la forma y por qué `schedule` (texto) se queda. Si cambias uno, cambia el otro.
-- ===========================================================================
...
-- ### 019_eventos
-- ###########################################################################
-- Apuntarse a un evento (`activity_attendees` + `activities.going_count`) y la imagen de
-- una campaña. Copia literal de db/13_eventos.sql, que explica cada pieza. Si cambias uno,
-- cambia el otro.
-- ===========================================================================
